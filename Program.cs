class Program
{
    static void Main(string[] args)
    {
        while (true)
        {
            ...
            if (operation != "+" && operation != "-")
            {
             ...
             Console.WriteLine("Enter the number of rows (mat) in the matrix:");
            int mat = Convert.ToInt32(Console.ReadLine());

            Console.WriteLine("Enter the number of columns (num) in the matrix:");
            int num = Convert.ToInt32(Console.ReadLine());

            float[,] matrix1 = new float[mat, num];
            float[,] matrix2 = new float[mat, num];
            float[,] resultMatrix = new float[mat, num];
    ...
    static double MatrixAddition(double[,] matrix, double number)
    {
